...
C---------------------------------------------------------
C Crack Planes: Normal along = [cos(45),  sin(45),      0];
C---------------------------------------------------------
C UNIAXIAL TENSILE LOADING CONDITIONS ONLY CONSIDERED
C Considering a stress loading in '2' direction
C
...
C We have 1 cracks: normal +45 degrees to '1'      

      T(1,:) = (/DCOS(PI/4.D0), DSIN(PI/4.D0), 0.0d0/)
...
C     write(*,*) 2.0d0*GIc*E/XT**2,L
      ...
      !check if element size is small enough (matrix)
      minExp2 = dmin1(2.0d0*GIc*E2/YT**2, 2.0d0*GIIc*G12/S12**2, 2.0d0*GIIIc*G23/S23**2)

      if ((minExp2).lt.L) then  
      write(*,*) ' '
      write(*,*) 'Element Size too large - 2'
      write(*,*) '(minExp2) - L'
      write(*,*) minExp2,L
      call myExit()      
      end if      
      
      minExp3 = dmin1(2.0d0*GIc*E2/ZT**2, 2.0d0*GIIc*G12/S13**2, 2.0d0*GIIIc*G23/S23**2)

      if ((minExp3).lt.L) then  
      write(*,*) ' '
      write(*,*) 'Element Size too large - 3'
      write(*,*) '(minExp3) - L'
      write(*,*) minExp3,L
      call myExit()      
      end if        
