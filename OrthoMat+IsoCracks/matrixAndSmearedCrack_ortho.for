...
C     !(6) - Mode I critical streess
C     !(7) - Mode I fracture toughness
C     !(8) - damping 
...
C     !(33)   - status for failure !!!
      ...
      sigcr0  = props(6)  !Mode I critical streess
      GIc     = props(7)  !Mode I fracture toughness
      damp    = props(8)  !damping 
...
C      call calcDcr(x,qepscrmax,qGIC,qsigcr0,qL,qDcr,qnu)
       call calcDcr(x,qepscrmax,qGIC,qsigcr0,qL,qDcr,qnu12)
...
C      call calcDcr(xPlusDx,qepscrmax,qGIC,qsigcr0,qL,qDcr,qnu)
       call calcDcr(xPlusDx,qepscrmax,qGIC,qsigcr0,qL,qDcr,qnu12)
...
C      call qcalcDcr(x,qepscrmax,qGIC,qsigcr0,qL,qDcr,qnu)
       call qcalcDcr(x,qepscrmax,qGIC,qsigcr0,qL,qDcr,qnu12)
      Fo=matmul(qDcr+qNT_Dco_N+qdti*qDda,x)
     #  -matmul(qNT,matmul(qDco,qeps))-matmul(qdti*qDda,qepscrold)  
      !check for convergence 
...
C      call qcalcDcr(xPlusDx,qepscrmax,qGIC,qsigcr0,qL,qDcr,qnu)
       call qcalcDcr(xPlusDx,qepscrmax,qGIC,qsigcr0,qL,qDcr,qnu12)
          Fn=matmul(qDcr+qNT_Dco_N+qdti*qDda,xPlusDx)
     #      -matmul(qNt,matmul(qDco,qeps))-matmul(qdti*qDda,qepscrold)
          J(:,k)=(Fn-Fo)/delta
          end do
...
C     subroutine calcDcr(epscr,epscrmax,GIC,sigcr0,L,Dcr,nu)
      subroutine calcDcr(epscr,epscrmax,GIC,sigcr0,L,Dcr,nu12)
      implicit none
...
C     term=1.0d0/(2.0d0*(1.0d0+nu))
      term=1.0d0/(2.0d0*(1.0d0+nu12))

      epscr_temp(4) = max(dabs(epscr(4)),dabs(epscrmax(2)))
...
C       end if
      ...
      Dcr(5,5)=Dcr(2,2)*term
      Dcr(6,6)=Dcr(2,2)*term
      Dcr(8,8)=Dcr(3,3)*term
      Dcr(9,9)=Dcr(3,3)*term
...
C     subroutine qcalcDcr(epscr,epscrmax,GIC,sigcr0,L,Dcr,nu)
      subroutine qcalcDcr(epscr,epscrmax,GIC,sigcr0,L,Dcr,nu12)
      implicit none
      
C     real*16::GIC,sigcr0,L,epscr(9),epscrmax(3),Dcr(9,9),nu,term
      real*16::GIC,sigcr0,L,epscr(9),epscrmax(3),Dcr(9,9),nu12,term
      real*16::epscr_temp(9),sigcr0_r, Dcr_min
      Dcr=0.0Q0
      
C     term=1.0Q0/(2.0Q0*(1.0Q0+nu))
      term=1.0Q0/(2.0Q0*(1.0Q0+nu12))

      epscr_temp(1) = max(qabs(epscr(1)),qabs(epscrmax(1)))
      epscr_temp(4) = max(qabs(epscr(4)),qabs(epscrmax(2)))
      epscr_temp(7) = max(qabs(epscr(7)),qabs(epscrmax(3)))
...
C       end if
     ...
     1 epscr_temp(7)+sigcr0)/epscr_temp(7)
      ...
      !construct the rest of the matrix
      Dcr_min = qmin1(Dcr(1,1), Dcr(4,4), Dcr(7,7))
      Dcr(1,1) = Dcr_min
      Dcr(4,4) = Dcr_min
      Dcr(7,7) = Dcr_min
      ...
      Dcr(5,5)=Dcr(4,4)*term*1.0D-5
      Dcr(6,6)=Dcr(4,4)*term*1.0D-5
      Dcr(8,8)=Dcr(7,7)*term*1.0D-5
      Dcr(9,9)=Dcr(7,7)*term*1.0D-5
